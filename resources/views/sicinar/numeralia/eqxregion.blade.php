...
@section('title','Estadística por region')
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>
        ...
        <small>Por Región</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Menú</a></li>
        <li><a href="#">Estadísticas de Equipos    </a></li>
        <li class="active">Por Región </li>
      </ol>
    </section>
              ...
              <table id="tabla1" class="table table-striped table-bordered table-sm">
                <thead style="color: brown;" class="justify">
                  <tr>
                    ...
                    <th rowspan="2" style="text-align:left;"  >REGIÓN </th>
                ...
                <tbody>
                  @foreach($regficha as $ficha)
                    <tr>
                      <td style="color:darkgreen;">{{$ficha->region_id}}</td>
                      <td style="color:darkgreen;">{{Trim($ficha->roma_region).' '.Trim($ficha->desc_region)}}</td>
                      <td style="color:darkgreen; text-align:center;">{{$ficha->total}}</td>
                    </tr>
                  @endforeach
                  @foreach($regfichaxreg as $totales)
                     <tr>
                        <td>                                  </td>
                        <td style="color:green;"><b>TOTAL</b> </td>                         
                        <td style="color:green; text-align:center;"><b>{{$totales->totalxregion}} </b></td>
                     </tr>
                  @endforeach
                </tbody>
              </table>
  ...
  <script type="text/javascript">
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ['Región', 'Total'],
          @foreach($regficha as $ficha)
             ['{{Trim($ficha->roma_region).' '.Trim($ficha->desc_region)}}', {{$ficha->total}} ],
          @endforeach          
        ]);
